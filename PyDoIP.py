    ...
    def RequestRoutingActivation(self, activationType=DEFAULT_ACTIVATION, localECUAddr=None, targetECUAddr=None):
        if self._isTCPConnected:
            try:
                ...
                payloadLength = "%.8X" % ((int)(len(payload) / 2))  # divide by 2 because 2 nibbles per byte  #zongliang comment it"""
                activationString = DoIPHeader + payloadLength + payload
                #print(type(activationString))
                ...
                self._TCP_Socket.send(bytes.fromhex(activationString))           #zongliang comment it here is the problem"""
                #self._TCP_Socket.send(activationString.encode())
                activationResponse = (binascii.hexlify(self._TCP_Socket.recv(2048))).upper()
                #print(type(activationResponse))
                if self._isVerbose:
                    print ("TCP RECV ::")
                ...
                #print( "Unable to activate routing with ECU:%.4X. Socket failed with error: %s" %(int(targetECUAddr), err))   #zongliang
                print("Unable to activate routing with ECU:%s. Socket failed with error: %s" %(targetECUAddr, err))
    ...
    def _DoIPUDSSend(self, message, localECUAddr=None, targetECUAddr=None, logging=True):
        if self._isTCPConnected:
            try:
                ...
                self._TCP_Socket.send(bytes.fromhex(UDSString))         #zongliang comment str to byte used to send socket
                return 0
            except socket.error as err:
...
#	testMsg = DoIPMsg(DOIP_PV+DOIP_IPV+DOIP_UDS+plLen+udspl+srcAddr+trgtAddr+'5001',verbose = True)
    ...
    DoIPClient.ConnectToDoIPServer()
    time.sleep(5)
    DoIPClient.ConnectToDoIPServer(defaultTargetIPAddr, 13400, True, '0C02')
